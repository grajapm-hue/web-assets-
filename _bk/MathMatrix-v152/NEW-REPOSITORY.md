...
Pairing the stock `sw.js` with the one-file copy makes it try to cache a dozen files that do not exist. It fails to install, and you silently get no offline play at all — the app still works online, so the fault is easy to miss.