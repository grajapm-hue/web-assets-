# 📁 MathMatrix Pro++ **v153** — your personal copy

Taken from the live site on **29 August 2026**.
...
|---|---|---|
| **`EDIT-THIS-MathMatrix-v153.html`** | 911 kB | ⭐ reading and editing. Opens fine in Markor. |
| **`PUBLISH-THIS-MathMatrix-v153.html`** | 5.6 MB | playing anywhere, and uploading to a site. One file, nothing beside it. |
| `index.html` | 911 kB | the live page, untouched — a true record. Leave it alone. |

Both copies are the same game. The difference is only where the pictures and
the music are kept.

- **EDIT** keeps them as separate files beside it. Longest line **755
...
| File | What it is |
|---|---|
| `EDIT-THIS-MathMatrix-v153.html` | ⭐ the one to work on |
| `PUBLISH-THIS-MathMatrix-v153.html` | the single-file copy for playing and publishing |
| `sw-standalone.js` | its partner **for publishing only** — this is what makes a published copy work with no signal |
| `index.html`, `sw.js`, `manifest.json` | the live site's own files, exactly as published |
...
| `MultiplyMagic3.html`, `sound-lab.html`, `design-preview.html` | the side pages on the live site |
...
| `RESTORE-AND-FORK.md` | how to put v153 back on the live site if something goes wrong |
| `WHATS-NEW-v153.md` | what changed since the v1.31 copy you already have |
| `SHA256SUMS.txt` | fingerprints, to prove nothing has been altered |

## Opening it on your phone

**To read or edit** — open `EDIT-THIS-MathMatrix-v153.html` in **Markor**, from
inside Markor's own file browser (not "Open with").

**To play** — open `PUBLISH-THIS-MathMatrix-v153.html` in **Chrome**. Because
...
- all eleven puzzles opened and were playable
- all eight formula sheets drew their own picture, from inside the file
