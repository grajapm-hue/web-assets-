...
|---|---|---|---|
| **`EDIT-THIS-MathMatrix-v153.html`** | 911 kB | 755 | changing words, tips, puzzle tables |
| **`PUBLISH-THIS-MathMatrix-v153.html`** | 5.6 MB | 676,117 | playing anywhere, and uploading to a site |
| `index.html` | 911 kB | 755 | the untouched original — leave it alone |

## Why two

The publish copy has the music, the eight formula sheets and the icons written
...
The edit copy keeps those as separate files beside it. Its longest line is 755
characters, so it opens instantly and behaves normally in Markor.

## The loop

1. **Edit** `EDIT-THIS-MathMatrix-v153.html` in Markor — or ask Claude to, see
...
3. **Change the version.** Find `var BUILD_VER = 'v153';` near the bottom and
   make it `v154`, or returning visitors keep the old copy from their cache.
4. **Rebuild the publish copy** — the single-file version with everything
   folded back in. Claude can do this, or see below to do it by hand.
...
**edit copy** as `index.html` and upload the loose picture files beside it —
that is exactly how the live site is arranged today. Use `sw.js`, not
`sw-standalone.js`, for that arrangement.
...
Find `var PUZZLES = {` (near line 5157). Each entry is one puzzle: its size, its
...
sum. Fill In The Blank grids must read **left to right** and have exactly one
answer.
