...
touching the existing live site and without needing anyone else.
...
|---|---|---|
| **Simple, one file** | `PUBLISH-THIS-MathMatrix-v153.html`, renamed to `index.html` | `sw-standalone.js` |
| **Same as the live site** | `EDIT-THIS-MathMatrix-v153.html`, renamed to `index.html` | `sw.js`, `manifest.json`, the 8 `cheat-*` files, `bgm-monkeys.mp3`, the 3 `icon-*` files |
...
2. Drag the files in
3. **Rename the game file to `index.html`** — click the file name after it
   uploads, or rename it on your computer before dragging
...
Upload the new file the same way, **and change `var BUILD_VER` inside it
first**. Without a new version number, everyone who has already visited keeps
the old copy from their cache and will never see the change. This is the single
most common reason an update "did not work".
...
Pairing the stock `sw.js` with the one-file copy makes it try to cache a dozen
files that do not exist. It fails to install, and you silently get no offline
play at all — the app still works online, so the fault is easy to miss.
