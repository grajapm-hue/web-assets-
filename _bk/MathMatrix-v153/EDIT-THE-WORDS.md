...
`EDIT-THIS-MathMatrix-v153.html`.
...
| the page itself | `MathMatrix Pro++` (7 places) | the name the child reads on screen |
...
`short_name` is currently **`Kids Maths`**. Keep it under about 12 characters
or Android trims it with no warning.
...
The tips and the wording for each puzzle live in `var PUZZLES = {` (near line
5157). Each entry has its own text. Change the words inside the quotes and
leave the punctuation around them alone.
...
| a different language | every quoted string | the app already has a language switch — see `var LANG =` (near line 13968) before starting a translation by hand |
...
3. Change `var BUILD_VER` to a new number before publishing.
