# 📁 MathMatrix Pro++ **beta-255** — your personal copy

Taken from the beta preview on **1 September 2026**.
...
| --- | --- | --- |
| **`EDIT-THIS-MathMatrix-beta-255.html`** | 887 kB | ⭐ reading and editing. Opens fine in Markor. |
| **`PUBLISH-THIS-MathMatrix-beta-255.html`** | 5.5 MB | playing anywhere, and uploading to a site. One file, nothing beside it. |
| `beta.html` | 887 kB | the published page, untouched — a true record. Leave it alone. |
...
| File | What it is |
| --- | --- |
| `EDIT-THIS-MathMatrix-beta-255.html` | ⭐ the one to work on |
| `PUBLISH-THIS-MathMatrix-beta-255.html` | the single-file copy for playing and publishing |
| `sw-standalone.js` | its partner **for publishing only** — this is what makes a published copy work with no signal |
| `beta.html`, `beta-sw.js`, `beta-manifest.json` | the published files, exactly as served |
...
| `RESTORE-AND-FORK.md` | how to put beta-255 back if something goes wrong |
| `SHA256SUMS.txt` | fingerprints, to prove nothing has been altered |

## Opening it on your phone

**To read or edit** — open `EDIT-THIS-MathMatrix-beta-255.html` in **Markor**, from inside Markor's own file browser (not "Open with").

**To play** — open `PUBLISH-THIS-MathMatrix-beta-255.html` in **Chrome**. Because everything is inside that one file, you can move it, rename it, mail it to yourself or drop it in Google Drive and it still works. No other file has to travel with it.
