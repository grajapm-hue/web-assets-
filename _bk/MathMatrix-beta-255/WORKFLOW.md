...
the site is arranged today. Use `beta-sw.js`, not `sw-standalone.js`, for that
arrangement.
...
Getting this pair wrong is the one mistake that matters: the stock `beta-sw.js`
...
> **Beta only — keep the name `beta.html`.** This page registers its worker as
> `beta-sw.js` with its scope pinned to `./beta.html`. If you rename the page to
...
> page named `beta.html` and visit `/beta.html`, or use the one-file copy — the
> single-file copy already had that scope removed when it was built.

## Adding a new puzzle

Find `var PUZZLES = {` (near line 5093). Each entry is one puzzle: its size, its
